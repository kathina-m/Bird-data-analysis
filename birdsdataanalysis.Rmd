...
mod1 <- lme(species_richness ~ category*size + canopy_cover + n_tree_spec + n_tree_ind + log(dbh_min) + n_microhabitats + temperature, random = (~1|site), data=dat, method="ML") #initial, full model with all potential predictor variables
summary(mod1)
...
mod2.1 <- lme(species_abund ~ category*size + canopy_cover + n_tree_spec + n_tree_ind + log(dbh_min) + log(n_microhabitats) + temperature, random = (~1|site), data=dat, method="ML") #initial, full model with all potential predictor variables
summary(mod2.1)
...
ggplot(dat, aes(x = log(dbh_min), y = species_abund))+
  geom_point() +
  geom_smooth(method = lm, se = TRUE, colour = 'green', size = 1.5) +
  geom_text(x=2.5, y=35, label="p-value: 0.0209", size=3.5) +
...
legend("topright",legend = c("Klosterpark Weende (20 species)", "Cheltenham Park (13 species)", "City Cemetery (16 species)", "City Forest (12 species)", "Forest Weende (12 species)", "Forest Schlucht (12 species)"), col = c("blue3", "deepskyblue", "deepskyblue3", "red2", "coral1", "pink"), cex = 0.7, pch = 16)
